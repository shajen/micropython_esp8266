...
class ThermostatServerController():
    ...
    def _update_working_mode(self):
        ...
        self._relay_pin.value((working_mode + 1) % 2)
    ...
    def process(self, url, params):
        if url == '/THERMOSTAT/':
            ...
            try:
                ...
                utils.printLog('THERMOSTAT', 'exception during process%s' % e)
    ...
    def update(self):
        try:
            ...
            utils.printLog('THERMOSTAT', 'exception during update %s' % e)
...
def initInstance(*args, **kwargs):
    global __INSTANCE__
    if not __INSTANCE__:
        __INSTANCE__ = ThermostatServerController(*args, **kwargs)
    return __INSTANCE__
